...
def on_activate(hotkey):
    ...
    if hotkey == '<ctrl>+<alt>+q':
        ...
        button_signal = "pause"
    elif hotkey == '<ctrl>+<alt>+t':
        button_signal = "taskbar"
    elif hotkey == '<alt>+<shift>+b':
        button_signal = "browser"
    elif hotkey == '<alt>+<shift>+m':
        button_signal = "music"
    elif hotkey == '<alt>+<shift>+f':
        button_signal = "files"
    elif hotkey == '<alt>+<shift>+v':
        button_signal = "vscode"
    elif hotkey == '<alt>+<enter>':
        button_signal = "terminal"
    elif hotkey == '<alt>+w':
        button_signal = "kill"
    elif hotkey == '<alt>+f':
        button_signal = "fullscreen"
    elif hotkey == '<alt>+<shift>+r':
        button_signal = "minimize"
...
def startListener():
    ...
    # Reset Drag State on listener restart
    drag_state["is_dragging"] = False
    drag_state["middle_held"] = False
    
    # Create listeners (Added on_move here)
    ...
    keyboard_listener = keyboard.GlobalHotKeys({
        ...
        '<ctrl>+<alt>+t': lambda: on_activate('<ctrl>+<alt>+t'),
        '<alt>+<shift>+b': lambda: on_activate('<alt>+<shift>+b'),
        '<alt>+<shift>+m': lambda: on_activate('<alt>+<shift>+m'),
        '<alt>+<shift>+f': lambda: on_activate('<alt>+<shift>+f'),
        '<alt>+<shift>+v': lambda: on_activate('<alt>+<shift>+v'),
        '<alt>+<enter>': lambda: on_activate('<alt>+<enter>'),
        '<alt>+f': lambda: on_activate('<alt>+f'),
        '<alt>+<shift>+r': lambda: on_activate('<alt>+<shift>+r'),
        '<alt>+w': lambda: on_activate('<alt>+w')
    })
