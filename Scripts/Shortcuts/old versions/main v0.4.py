...
def on_scroll(x, y, dx, dy):
    global button_signal, listener_running, alt_mode

    if not alt_mode:
        return

    if dy > 0: # Alt + Scroll Up
        button_signal = "alt_scroll_up"
        listener_running = False
        return False

    elif dy < 0: # Alt + Scroll Down
        button_signal = "alt_scroll_down"
        listener_running = False
        return False
...
def yasb():
    ...
    if 'YasbBar' in windows:
        ...
        taskbar()
        os.startfile('yasb')
        os.startfile(r"C:\ProgramData\Microsoft\Windows\Start Menu\Programs\GlazeWM.lnk")
...
def toggle_fullscreen():
    ...
    if is_window_maximized(hwnd):
        win32gui.ShowWindow(hwnd, win32con.SW_NORMAL)
        print(f'{hwnd} unfullscreened')
    else:
        #win32gui.ShowWindow(hwnd, win32con.SHOW_FULLSCREEN)
        win32gui.ShowWindow(hwnd, win32con.SW_MAXIMIZE)
        print(f'{hwnd} fullscreened')

# minimize
def minimize():
    hwnd = win32gui.GetForegroundWindow()
    win32gui.ShowWindow(hwnd, win32con.SW_MINIMIZE)
    print(f'{hwnd} minimized')
    return hwnd
    ...
    def main():
        while True:
            ...
            if filtered_windows:
                ...
                break
            force_close_windows(filtered_windows) # hwnd
...
def main():
    ignore = ['Zen', 'Explorer', 'CapCut', 'Chrome', 'Select exporting path', 'Spotify', 'File Upload']
    while True:
        ...
        if signal == 'pause': # alt ctrl p
            ...
            if 'YasbBar' in windows:
                ...
                while 'YasbBar' in windows:
                    ...
                    time.sleep(0.1)
            p.moveTo(x=1915, y=1079)
            p.click(x=1915, y=1079)
            time.sleep(1)
        ...
        elif signal == 'debug': # alt ctrl d
            print('reloading with terminal')
            os.system(r'start "" py "C:\Users\Sweetwaters Church\Jordan\Scripts\Shortcuts\main.py"')
            exit()
        ...
        elif signal == "alt_scroll_up":
            print("ALT + SCROLL UP triggered")
            p.hotkey('alr', 's')

        elif signal == "alt_scroll_down":
            print("ALT + SCROLL DOWN triggered")
            p.hotkey('alt', 'a')
