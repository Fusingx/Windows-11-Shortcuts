...
CONFIG = {
    ...
    "black_cursor_dir": r"C:\Windows\Cursors\arrow_r.cur", 
    "white_cursor_dir": r"C:\Windows\Cursors\aero_arrow.cur",
    "paths": {
...
class AutomationEngine:
    ...
    def find(self, image, timeout=1.0):
    ...
    def exec_taskbar_toggle(self):
        ...
        behaviors = self.find('behaviors.jpg', 2)
        if behaviors:
            p.moveTo(behaviors)
            time.sleep(0.2)
            p.click(behaviors)
    ...
    def exec_shutdown_routine(self):
        ...
        p.press('up')
        p.press('enter')
        p.click(self.find('dontstart.jpg'))
        time.sleep(0.1)
        p.press('right')
        p.press('enter')
    ...
    def exec_random_wallpaper(self):
        ...
        try:
            if 'PowerPoint' in title:
                if button == 'x1':
                    ...
                    p.hotkey('ctrl', 'a')
                    mouse_pos = p.position()
                    if loc := self.find('paste.jpg'): p.click(loc)
                    p.moveTo(mouse_pos)
                ...
                if button == 'x1': p.press('r')
                    ...
                    orig_pos = p.position()
                    if loc := self.find('notcrop.jpg'): p.click(loc)
                    p.press('r')
                    if loc := self.find('original.jpg'): 
                        p.click(loc)
    ...
    def run(self):
        ...
        self.exec_toggle_cursor(colour=CONFIG['black_cursor_dir'])
    ...
    def handle_global_command(self, cmd):
        ...
        elif cmd == 'wallpaper': self.exec_random_wallpaper()
