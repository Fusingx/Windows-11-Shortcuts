...
today_date = datetime.now().strftime("%Y-%m-%d") # e.g., "2026-03-11"
day_name = datetime.now().strftime("%A")        # e.g., "Wednesday"
...
def load_data():
    """Loads saved states and checks if it's a new day."""
    if os.path.exists(DATA_FILE):
        try:
            with open(DATA_FILE, "r") as f:
                data = json.load(f)
                # If the date in the file matches today, return the task states
                if data.get("date") == today_date:
                    return data.get("tasks", {})
        except (json.JSONDecodeError, KeyError):
            pass 
    
    # If file doesn't exist or it's a new day, return empty states (reset)
    return {}

def save_data():
    """Saves the current date and checkbox states."""
    data_to_save = {
        "date": today_date,
...
for task_text in current_tasks:
    # Use the saved state ONLY if it's from today
...
if root.winfo_exists():
    save_data()  # Save your checklist one last time
    root.destroy()
else:
    pass